  ...
  createElement() {
    ...
    if (!this.local.loggedIn) {
      return html`
        <send-account>
          <button
            class="px-4 py-2 md:px-8 md:py-4 focus:outline signin border-2 link-blue border-blue-60 dark:border-blue-40 dark:hover:border-blue-50"
    ...
    return html`
      <send-account class="relative h-8">
        ...
        <ul
          id="accountMenu"
          class="invisible absolute top-0 right-0 mt-10 pt-2 pb-2 bg-white shadow-md whitespace-no-wrap outline-none z-50 dark:bg-grey-90"
          onblur="${e => this.hideMenu(e)}"
        >
          <li class="p-2 text-grey-60">${user.email}</li>
          <li>
            <button
              class="block w-full text-left px-4 py-2 text-grey-80 hover:bg-blue-50 hover:text-white cursor-pointer focus:outline"
